...
export default function FilterTabs({ activeTab, onTabChange }: Props) {
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    label: { fontSize: 17, fontWeight: '500', color: C.subtext },
    activeLabel: { color: Colors.accent, fontWeight: '700' },
    activeUnderline: { position: 'absolute', bottom: 0, left: 0, right: 0, height: 2, backgroundColor: Colors.accent, borderRadius: 2 },
    fullBorder: { height: 1, backgroundColor: C.border },
  }), [C]);
